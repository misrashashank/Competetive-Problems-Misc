...
Every employee has an Id, a salary, and there is also a column 
...
SELECT d.Name AS Department, e.Name AS Employee, e.Salary
FROM Employee e, 
    (SELECT DepartmentId, MAX(Salary) AS MAX_SALARY FROM Employee GROUP BY DepartmentId) x,
    Department d
WHERE e.DepartmentId = d.Id
    AND d.Id = x.DepartmentId
